	...
	$.fn.extend({ 
		infiniteCarousel: function(options)
		{
			...
			var options = $.extend(defaults, options);
				...
				for(var i=0;i<numItems;i++) $('div.ic_tray',obj).append('<div style="overflow:hidden;background:url('+o.imagePath+') no-repeat scroll 50% 50% ;float:left;position:relative;width:'+widthSource+'px;height:'+heightSource+'px;" class="infiniteCarousel_item">');
				...
				if(o.thumbnailType != 'none')
				{
					...
					$(obj).append('<div id="ic_'+randID+'_thumbnail_tray" class="ic_thumbnail_tray" style="position:absolute;bottom:0;margin-top:100px;width:100%;text-align:center;">');
					...
					$('.ic_button').hover(function(){ if( !$(this).hasClass('ic_active') ) $(this).css('background','#ff5912') },function(){$(this).css('background','')});

					if(o.thumbnailType == 'image')
					{
						...
						$('.ic_button',obj).each(function(index){
							...
							$(this).append('<img width="50%" height="100%"  src="'+thumbImage+'" />');
						});
					}
				...
				if(canvasSupported)
				{
					$(obj).append('<canvas id="ic_canvas_'+randID+'" width="0" height="30" style="position:absolute;top:0;right:0;"></canvas>');
